<?php
   ...
   $fechaInicio=$_GET["start-date"];
   $fechaFinal=$_GET["end-date"];

?>       
<?php
   
if (!empty($_GET["start-date"]) and !empty($_GET["end-date"])) {
   ...
   class PDF extends FPDF
   {
      ...
      function Header()
      {
         ...
         $this->Cell(90);  // mover a la derecha
         $this->SetFont('Arial', 'B', 10);
         $this->Cell(85, 10, utf8_decode("Fecha de reporte : ".$_GET["start-date"]." - ".$_GET["end-date"]), 0, 0, '', 0);
         $this->Ln(10);
